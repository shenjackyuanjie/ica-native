...
impl IcaApp {
    pub(super) fn render_message_search_window(&mut self, ctx: &egui::Context) {
        ...
        if let Some(action) = pending_action {
            self.handle_message_search_action(active_bridge_idx, room_id, &messages, action);
        }
    }
    ...
    fn handle_message_search_action(
        ...
        match action {
            ...
            MessageAction::PreviewImage { url } => {
                let images = IcaApp::search_result_image_urls(search_messages);
