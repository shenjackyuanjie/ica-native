...
use crate::app::media::ImageAction;
...
impl IcaApp {
    pub fn render_image_viewer(&mut self, ctx: &egui::Context) {
        ...
        let pending_action = self
            .image_viewer
            .as_ref()
            .and_then(|viewer| viewer.lock().unwrap().pending_action.take());
        if let Some(action) = pending_action {
            if let Some(bridge_idx) = self.active_bridge_idx {
                self.handle_image_action(ctx, bridge_idx, action);
            } else {
                self.media_error = Some("没有可处理图片动作的活动 bridge".to_string());
            }
        }
                ...
                egui::Panel::top("image_viewer_toolbar").show(ui, |ui| {
                    ui.horizontal_wrapped(|ui| {
                        ...
                        if ui.button("复制图片").clicked() {
                            ...
                            state.pending_action = Some(ImageAction::CopyImage(source));
                        }
                        if ui.button("复制 URL").clicked() {
                            let mut state = viewer_state.lock().unwrap();
                            let source = state.current_source();
                            state.pending_action = Some(ImageAction::CopyUrl(source));
                        }
                        if ui.button("保存").clicked() {
                            let mut state = viewer_state.lock().unwrap();
                            let source = state.current_source();
                            state.pending_action = Some(ImageAction::Save(source));
                        }
                        if ui.button("另存为…").clicked() {
                            let mut state = viewer_state.lock().unwrap();
                            let source = state.current_source();
                            state.pending_action = Some(ImageAction::SaveAs(source));
                        }
                        let can_locate = {
                            let source = viewer_state.lock().unwrap().current_source();
                            source.room_id.is_some() && source.message_id.is_some()
                        };
                        if ui
                            .add_enabled(can_locate, egui::Button::new("在聊天中定位"))
                            .clicked()
                        {
                            let mut state = viewer_state.lock().unwrap();
                            let source = state.current_source();
                            state.pending_action = Some(ImageAction::Locate(source));
                        }
                        ui.separator();
